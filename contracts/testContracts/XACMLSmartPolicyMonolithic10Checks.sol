...
contract XACMLSmartPolicyMonolithic10Checks {
    ...
    constructor(Verifier10 _verifier) {
    ...
    function evaluateCondition(address _subject, Verifier10.Proof memory _proof) public view returns (bool) {
        ...
        uint[] memory _inputs = AMContract(0x1dd0B5b3E207C7fB0A8dE897E89c1f85E487c5E0).getMetadataOf(_subject, "Verifier10");

        if(_inputs.length != 20)
            return false;

        if(verifier.verifyTx(_proof, [_inputs[0], _inputs[1], _inputs[2], _inputs[3], _inputs[4], _inputs[5], _inputs[6], _inputs[7], _inputs[8], _inputs[9], _inputs[10], _inputs[11], _inputs[12], _inputs[13], _inputs[14], _inputs[15], _inputs[16], _inputs[17], _inputs[18], _inputs[19], 27, 1]))
            return true;
        else
            return false;
    }
